...
extension Game {
    ...
    internal func placeShips() -> [Ship] {
        let ships: [Ship] = ShipClassification.allCases.map { classification in
    ...
    /// - localizationKey: Game.placeShip(_:)
    private func placeShip(_ ship: Ship) {
        ...
        case .horizontal:
            ...
            repeat {
                start = Coordinate(column: Int.random(in: 0 ..< grid.columnCount), row: Int.random(in: 0 ..< grid.rowCount))
    ...
    /// - localizationKey: Game.isShipPlaceable(from:to:)
    ...
    /// - localizationKey: Game.isShipAt(_:)
