...
public class SpiralViewController: UIViewController, PlaygroundLiveViewSafeAreaContainer {
    ...
    /// Minimum scale to which the user may zoom
    private let maxScaleLimit: CGFloat = 4
    /// Maximum scale to which the user may zoom
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        // Internal note: you may be seeing an error that .default has been renamed to .defaultRunLoopMode. The fixit is a lie, see 48361451 for more details.
    ...
    override public func viewDidLayoutSubviews() {
        ...
        if oldBounds != liveViewSafeAreaGuide.layoutFrame {
            ...
            spiralView?.configureShapeLayers()
            spiralView?.setNeedsDisplay()
    ...
    fileprivate func addAndConstrainSpiralView() {
        ...
        widthConstraint = spiralView.widthAnchor.constraint(equalToConstant: view.bounds.width)
        heightConstraint = spiralView.heightAnchor.constraint(equalToConstant: view.bounds.height)
        widthConstraint.priority = UILayoutPriority(rawValue: 0.750)
        heightConstraint.priority = UILayoutPriority(rawValue: 0.750)
        
        let widthBoundingConstraint = spiralView.widthAnchor.constraint(lessThanOrEqualToConstant: view.bounds.width * 0.75)
        let heightBoundingConstraint = spiralView.heightAnchor.constraint(lessThanOrEqualToConstant: view.bounds.height * 0.75)
        ...
        NSLayoutConstraint.activate([widthConstraint,
                                     heightConstraint,
                                     widthBoundingConstraint,
                                     heightBoundingConstraint,
                                     centerYConstraint,
                                     centerXConstraint])
